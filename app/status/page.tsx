      ...
      <Suspense fallback={<Loading />}>
        ...
        <div className="rounded-box bg-base-200 mb-10 flex flex-col justify-center">
          <div className="flex flex-col gap-4 max-w-[56rem] my-5 text-center px-10">
            ...
            <h2 className="text-xl">Bot Status: {botStatus.status === "running" ? <span className=" text-success">Running</span> : <span className=" text-error">Offline</span>} </h2>
            <h2 className="text-xl">Bot Uptime: {botStatus.status === "running" ? prettyMilliseconds(botStatus.upTime, {compact: true}) : <span className="text-error">Offline</span>} </h2>
